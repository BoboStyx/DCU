...
=======
            ...
            *output_file = args[i + 1];//Get file name
            args[i] = NULL;//Remove input symbol
            *append = 0;//Ensure append is false
            *out_fd = open(*output_file, O_WRONLY | O_CREAT | O_TRUNC, 0644);//Get file description and writes one, creates if no file exists, and truncates if there's already information in file also sets the files
 permissions if new file created.
>>>>>>> dba6c744c9a9dc6030a1a7e486460f306e60a294
...
=======
            ...
            *output_file = args[i + 1];//Get file name.
            args[i] = NULL;//Remove input symbol.
            *append = 1;//Ensure append is true.
            *out_fd = open(*output_file, O_WRONLY | O_CREAT | O_APPEND, 0644);//Get file description and writes one, creates if no file exists, and appendss if there's already information in file also sets the permi
ssions if new file created.
>>>>>>> dba6c744c9a9dc6030a1a7e486460f306e60a294
            if (*out_fd == -1){//Error case
                perror("Output redirection failed");
                exit(1);
